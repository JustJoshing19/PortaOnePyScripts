...
logging_name = "Update_timeout_" + str(log_time)
...
def update_service(token: str, acc_list: list[dict]) -> bool:
    """
    ...
    update_list = [acc["i_account"] for acc in acc_list]
    ...
    for acc in update_list:
        ...
        logger.info(f"Updated timeout of account {acc}")

    logger.info("All accounts' timeouts updated.")
    return True
